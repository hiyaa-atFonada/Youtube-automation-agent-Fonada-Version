...
const { resolveFonadaLanguage } = require('../utils/fonada-tts');
...
class ProductionManagementAgent {
  ...
  async generateAudioNarration(productionData) {
    ...
    try {
      ...
      const language = resolveFonadaLanguage({
        explicit: productionData.script?.language || productionData.strategy?.language,
        text: ttsText
      });
      
      // Generate audio using Fonada (or the remaining TTS fallbacks)
      await this.aiVideoGenerator.generateTTSAudio(ttsText, audioPath, {
        language: language.iso,
        voice: productionData.strategy?.voice || productionData.script?.voice,
        voiceRecord: productionData.strategy?.voiceRecord || null
      });
      const narration = this.aiVideoGenerator.lastNarration || {};
      
      productionData.assets.audio = {
        path: audioPath,
        duration: productionData.estimatedDuration,
        format: 'mp3',
        generatedWith: narration.provider || 'AI',
        language: narration.language?.iso || language.iso,
        quality: 'high'
      };
  ...
  async generateCaptions(productionData) {
    ...
    productionData.assets.captions = {
      path: captionsPath,
      format: 'srt',
      language: productionData.assets.audio?.language || productionData.script?.language || 'en',
      autoGenerated: true
    };
  ...
  async assembleVideo(productionData) {
    ...
    try {
      ...
      const actualSeconds = await getMediaDuration(finalVideoPath);
      
      productionData.assets.finalVideo = {
        path: finalVideoPath,
        fileSize: stats.size,
        duration: actualSeconds
          ? `${Math.floor(actualSeconds / 60)}:${String(Math.round(actualSeconds % 60)).padStart(2, '0')}`
          : productionData.estimatedDuration,
        generatedWith: 'AI',
        resolution: '1920x1080',
        format: 'mp4'
      };
